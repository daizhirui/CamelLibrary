...
float FP_UnsignedToFloat(unsigned long af)
{
    unsigned long a;
    unsigned long as = 0;
...
{
    long int_part = FP_ToLong(a_fp);
    long frac_part = FP_ToLong((a_fp - int_part) * 10000.0);
    
    puts(num2Dec(int_part));
    putch('.');
    puts(num2Dec(frac_part));
}
