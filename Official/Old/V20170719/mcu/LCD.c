...
void RT_LCD_DisHiPressure(unsigned int b) 		//display systolic pressure					 
{	
	int tmp2,tmp1,tmp;
	...
	else
	{
		...
		if(tmp2!=0||tmp1!=0)		//display the tens digit;if both hundreds digit and tens digit are equal to 0,don't display
		  {	RT_LCD_GetSegCode1(tmp1);//change from d10 to digit[1]
			for(i=0;i<4;i++)				
			{	
				segtmp=MemoryRead32(segment+i*4);
				d=MemoryRead32(LCD_RAM_REG+i*4);	
...
void RT_LCD_DisLoPressure(unsigned int b)	//display diastolic pressure			 
{	unsigned int i;
	unsigned long d;
	int tmp2,tmp1,tmp;
	unsigned long segtmp;
...
void RT_LCD_DisHeartRate(unsigned int b)			//display heart rate
...
void RT_LCD_DisTime(unsigned int hour,unsigned int minute)	//display time
{	unsigned int i;
	unsigned long d;
	int segtmp;
	d=MemoryRead32(LCD_RAM_REG);		
	...
	//RT_LCD_DisSign(colon);			//display������
	if(hour>12)				//display in duodecimal system
	{	hour=hour%12;
		RT_LCD_DisSign(PM);		//pm=1
		RT_LCD_ClearSign(AM);
	}
	else
	{	RT_LCD_DisSign(AM);		//am=1;
		RT_LCD_ClearSign(PM);
	}		
...
void RT_LCD_DisDate(unsigned int month,unsigned int day)		//display day
{	unsigned int i;
	unsigned long d;
	int segtmp;
	...
	{	
		RT_LCD_DisSign(B13);
		RT_LCD_DisSign(C13);
		//RT_LCD_DisSign(M1);		//M1=1;
	}
	RT_LCD_GetSegCode3(month%10);
	for(i=0;i<4;i++)
	{	
		segtmp=MemoryRead32(segment+i*4);
		d=MemoryRead32(LCD_RAM_REG+i*4);	
...
void RT_LCD_DisSign(unsigned int x,unsigned int y)		//display special sign
{	
...
void RT_LCD_ClearSign(unsigned int x,unsigned int y)		//clear special sign
{	unsigned long c=0x00000001,d;
	d=MemoryRead32(LCD_RAM_REG+x);
	d=d&~(c<<y);
	MemoryWrite32(LCD_RAM_REG+x,d);
...
void RT_LCD_GetSegCode1(unsigned int b)		//get the display segment code,red part of the truth table 
{	unsigned int i;
	unsigned long d;
	unsigned long tmp;
	unsigned long segtmp;
...
void RT_LCD_GetSegCode2(unsigned int b)		//get the display segment code,red part of the truth table
{	unsigned int i;
	unsigned long d;
	unsigned long tmp;
	unsigned long segtmp;
...
void RT_LCD_GetSegCode3(unsigned int b)		//get the display segment code,green part 
{	unsigned int i;
	unsigned long d;
	unsigned long tmp;
	unsigned long segtmp;
...
void RT_LCD_BlinkSign(unsigned int x,unsigned int y)		//blink special sign
{	unsigned long c=0x00000001,d;
	int a;
