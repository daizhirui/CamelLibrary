...
/************* OPO setup **************/
// set OPO on
#define OPO_On()    MemoryOr32(AD_OPO_REG, 0x1)
// set OPO off
#define OPO_Off()   MemoryAnd32(AD_OPO_REG, ~1)
/**
 * @brief 
 ...
 * @return          void
 */
#define OPO_SetChannel(ch0, ch1, ch2, ch3)                                           \
    {                                                                                \
 ...
 * @return          void
 */
#define OPO_SetAmplification(Pin, Pgain, Nin, Ngain)                                     \
    {                                                                                    \
 ...
 * @return          void
 */
#define OPO_SetPsideFeedback(mode)              \
    {                                           \
        if (mode)                               \
 ...
 * @return          void
 */
#define OPO_SetSingleSideMode(PorN, channel)  \
{                                             \
    ADC_V2P_SetRes(V2P_185K);                 \
    ADC_TemperatureSensorOff();               \
    OPO_SetPNExchange(PorN);                  \
    MemoryAnd32(AD_OPO_REG, ~(0xf << 4));     \
 ...
 * @return          void
 */
#define OPO_SetDifferentialMode(PorN, channel) \
{                                              \
    ADC_V2P_SetRes(0);                         \
    OPO_SetPNExchange(PorN);                   \
    MemoryAnd32(AD_OPO_REG, ~(0xf << 4));      \
...
/************* SD Setup **************/
// set ADC_SD on
#define ADC_SD_On()     MemoryOr32(AD_CTL0_REG, 1)
// set ADC_SD off
#define ADC_SD_Off()    MemoryAnd32(AD_CTL0_REG, ~1)
/**
 * @brief This function sets the samplerate of ADC_SD
 * 
 * @param mode      SD_CLK_3M, SD_CLK_1_5M, SD_CLK_781K or SD_CLK_390K
 * @return          void
 */
#define ADC_SD_SetSampleRate(mode)             \
    {                                          \
 ...
 * @return          void
 */
#define ADC_SD_SetFilterCLK(freq)              \
    {                                          \
 ...
 * @return          void
 */
#define ADC_SD_SetAdBits(mode)                 \
    {                                          \
        MemoryAnd32(AD_CTL0_REG, ~(0x3 << 1)); \
        MemoryOr32(AD_CTL0_REG, (mode << 1));  \
    }
/**
 * @brief This function sets the trigger source of ADC_SD
 * 
 * @param op        SD_PWM or SD_WT2READ
 * @return          void
 */
#define ADC_SD_SetTrigger(op)         \
{                                     \
    MemoryAnd32(AD_CTL0_REG,~(1<<6)); \
    MemoryOr32(AD_CTL0_REG,op<<6);    \
}
/**
 * @brief This function sets ADC_SD briefly.
 * 
 * @param samplerate    the samplerate of SD
 * @param ad            the number of ad
 * @return              void
 */
#define ADC_SD_Setup(samplerate, ad)  \
{                                     \
    ADC_SD_SetSampleRate(samplerate); \
    ADC_SD_SetAdBits(ad);             \
}
/**
...
/************* V2P Setup **************/
// set ADC_V2P on
#define ADC_V2P_On()    MemoryOr32(AD_CTL0_REG, (1 << 8))
// set ADC_V2P off
#define ADC_V2P_Off()   MemoryAnd32(AD_CTL0_REG, ~(1 << 8))
/**
 ...
 * @return #define 
 */
#define ADC_V2P_SetRes(res)                    \
    {                                          \
        MemoryAnd32(AD_CTL0_REG, ~(0x3 << 9)); \
        MemoryOr32(AD_CTL0_REG, (res << 9));   \
    }
// set ADC temperature sensor on
#define ADC_TemperatureSensorOn()   MemoryOr32(AD_CTL0_REG, 1 << 5)
// set ADC temperature sensor off
#define ADC_TemperatureSensorOff()  MemoryAnd32(AD_CTL0_REG, ~(1 << 5))
/**
